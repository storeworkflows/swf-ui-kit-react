...
const Preloader = React.forwardRef((props, ref) => {
  const {
    count, items, width, height, flexDirectionGeneral, background, mainStyles, blur,
  } = props;

  return (
    <div
      className="swf-preloader"
      style={{ flexDirection: flexDirectionGeneral, flexWrap: 'wrap' }}
      ref={ref}
    >
      {new Array(count).fill(null).map((el, i) => (
        <div
          className={classNames({ 'mock-container': true, gray: background })}
          style={({ width, height, ...mainStyles })}
        ...
        >
          ...
          {items.map(({
            repeat, width, height, styles, itemStyles,
          }, i) => (
            <div
