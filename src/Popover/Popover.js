...
const Popover = React.memo(React.forwardRef((props, ref) => {
  const {
    children, roundBorder, contentStyles, positionTarget, manageOpened,
    onTargetClick, onOuterPopoverClicked, positions, hideTail, opened,
  } = props;
  ...
  const documentClicked = (event) => {
    ...
    if (isOpened && contentElement && targetElement) {
      const isOutsideContent = !isPointInsideTheElement(contentElement, pointX, pointY);
      const isOutsideTarget = !isPointInsideTheElement(targetElement, pointX, pointY);
  ...
  const setStyles = (isHidden, popoverStyles) => {
    ...
    Object.entries(stylesToSet).map(([key, value]) => {
      if (key === 'maxHeight' || key === 'maxWidth') contentElement.children[0].style[`${key}`] = value;
    ...
    if (isHidden) {
      ...
      Object.entries(popoverStyles.arrowStyle).map(([key, value]) => contentElement.style[`${key}`] = value);
    }
  };
  ...
  const calculateStyles = () => {
    ...
    if (contentElement && targetElement) {
      ...
      const stylesInfo = getPopoverStyle(positions, targetDimensions, contentDimensions, hideTail, roundBorder, padding);
