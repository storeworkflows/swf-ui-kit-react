/*! RESOURCE: /scripts/sn-component-custom-tinymce/themes/modern/theme.js */
(function () {
    ...
    item.size = size;
    item.contextElm = editor.contextElm, item.target = editor.targetElm;
    ...
    var containerElm, iframeElm, containerSize, iframeSize, contentArea;
    containerElm = editor.getContainer();
    contentArea = editor.getContentAreaContainer();
    iframeElm = contentArea.firstChild;
    containerSize = getSize(containerElm);
    iframeSize = getSize(iframeElm);
    if (width !== null && width !== contentArea.clientWidth) {
    ...
    self._elmCache = {};
    self.$ = global$9.overrideDefaults(function () {
    return { context: self.settings.contextElm };
    });
    ...
    this._elmCache[id] = this.$('#' + id, this.settings.target.parentNode)[0];
    }
    if (!this._elmCache[id]) {
    this._elmCache[id] = this.$('#' + id)[0];
    }
    }
    ...
    this.$(elm).before(this.renderHtml());
    this.postRender();
    return this;
    },
    renderTo: function (elm) {
    this.$(elm || this.getContainerElm()).append(this.renderHtml());
    this.postRender();
    return this;
    },
    preRender: function () {
    ...
    eventCtrl.$(eventCtrl.getEl()).on('mousewheel', fixWheelEvent);
    } else {
    eventCtrl.$(eventCtrl.getEl()).on('DOMMouseScroll', fixWheelEvent);
    }
    continue;
    }
    if (name === 'mouseenter' || name === 'mouseleave') {
    if (!eventRootCtrl._hasMouseEnter) {
    eventCtrl.$(eventRootCtrl.getEl()).on('mouseleave', mouseLeaveHandler).on('mouseover', mouseEnterHandler);
    eventRootCtrl._hasMouseEnter = 1;
    }
    } else if (!eventRootDelegates[name]) {
    eventCtrl.$(eventRootCtrl.getEl()).on(name, delegate);
    ...
    contextElm: self.settings.contextElm,
    ...
    function DragHelper (id, settings, shadowRoot) {
    var $eventOverlay;
    var doc = settings.document || domGlobals.document;
    var downButton;
    var start, stop, drag, startX, startY;
    settings = settings || {};
    var handleElement = (shadowRoot || doc).getElementById(settings.handle || id);
    start = function (e) {
    ...
    self.aria('label', self.translate(settings.ariaLabel || settings.tooltip));
    },
    tooltip: function () {
    if (!this._tooltip) {
    ...
    self.menu = global$4.create(menu, {
    contextElm: self.settings.contextElm,
    target: self.settings.target
    ...
    return window.tinymce && window.tinymce.activeEditor ? window.tinymce.activeEditor : global$1.activeEditor;
    };
    var history = {};
    ...
    menu = self.menu = global$4.create(menu, {
    contextElm: self.settings.contextElm,
    target: self.settings.target
    }).parent(self).renderTo();
    menu.reflow();
    ...
    }, self.getEl().getRootNode ? self.getEl().getRootNode() : undefined);
    },
    remove: function () {
