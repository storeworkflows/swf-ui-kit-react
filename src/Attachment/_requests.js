...
const errorMessage = (error, delay) => {
  ...
  return {
    systemMessages: [{ content: error.toString(), icon: errorIcon, delay }],
  };
};

export const downloadRequest = async (attachmentSysId, delay) => {
  if (attachmentSysId) {
    const url = `/api/now/attachment/${attachmentSysId}`;
...
export const deleteRequest = async (attachmentSysId, delay) => {
  if (attachmentSysId) {
    ...
    try {
      const response = await fetch(url, requestOptions);
      if (!response.ok) return errorMessage(response.statusText, delay);
    } catch (error) {
      console.log(error);
    }
  }
