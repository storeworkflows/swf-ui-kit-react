...
const RangePicker = React.forwardRef((props, ref) => {
  ...
  const changeValue = (e, isFirst) => {
    ...
    const newValue = addCharToDate(format, value, input);

    const updatedDates = isFirst
  ...
  const onSelected = useCallback(({ updated }) => {
    ...
    if (isFirstSelecting && !updatedDates.end) setIsFirstSelecting(false);
    else if (!isFirstSelecting && !updatedDates.start) setIsFirstSelecting(true);
    else openCalendar();
  }, [isOpenedInFirst, format, changeSelectedValue, setValue, openCalendar]);

  const invalidInput = (selectedDates) => {
    const errors = selectedDates ? getErrors(selectedDates, format, min, max) : [];
  ...
  const renderRangeCalendar = () => {
    ...
    return popoverTarget && Boolean(openedDate) && (
    <Popover
      hideTail
      manageOpened
  ...
  return visible && (
  <div ref={ref} className="range-picker-container">
