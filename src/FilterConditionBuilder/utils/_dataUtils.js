export const DATA_UTILS = {
  getDropdownFieldsItems: ({
    tableFields, index, blockFields, allowFileds, listIndex,
  }) => {
      ...
      if (tableFields[key].operators) {
        if (blockFields && blockFields.length) {
          !blockFields.find((name) => tableFields[key].name === name) && dropdownFields.push({
            id: tableFields[key].name, label: `${tableFields[key].label}`, dropdown: 'field', index, reference: `${tableFields[key].type === 'reference'}`, table: tableFields[key].reference, firstOperator: { operator: tableFields[key].operators[0].operator, editor: tableFields[key].operators[0].advancedEditor } || '',
          });
          ...
          dropdownFields.push({
            id: tableFields[key].name, label: `${tableFields[key].label}`, dropdown: 'field', index, listIndex, reference: `${tableFields[key].type === 'reference'}`, table: tableFields[key].reference, firstOperator: { operator: tableFields[key].operators[0].operator, editor: tableFields[key].operators[0].advancedEditor } || '',
          });
        }
      }
    }
          ...
          if (tableFields[key].type === tableFields[field].type && JSON.stringify(tableFields[key]) !== JSON.stringify(tableFields[field])) {
            if (editor === 'glide_date_equivalent') {
              valueAdditionalData.push({
                id: tableFields[key].name, label: tableFields[key].label, dropdown: 'value', type: 'glide_date_equivalent', index: '1',
              });
            } else if (editor === 'glide_date_comparative') {
              valueAdditionalData.push({
                id: tableFields[key].name, label: tableFields[key].label, dropdown: 'value', type: 'glide_date_comparative', index: '3',
              });
            } else {
    ...
    switch (item) {
      case 'dayofweek':
        trendData = moment.weekdays().map((day) => ({
          id: day.toLowerCase(), label: day, dropdown: 'value', type: 'trend_field', index: '2',
        }));
        break;
      case 'month':
        trendData = moment.months().map((month) => ({
          id: month.toLowerCase().slice(0, 3), label: month, dropdown: 'value', type: 'trend_field', index: '2',
        }));
        break;
      case 'quarter':
        for (let i = 1; i < 5; i++) {
          trendData.push({
            id: `${i}`, label: `Quarter${i}`, dropdown: 'value', type: 'trend_field', index: '2',
          });
        }
        break;
      case 'year':
        for (let i = moment().year() - 20; i < moment().year() + 10; i++) {
          trendData.push({
            id: `${i}`, label: i, dropdown: 'value', type: 'trend_field', index: '2',
          });
        }
        break;
      case 'week':
        for (let i = 0; i < 54; i++) {
          trendData.push({
            id: `${i}`, label: `Week${i}`, dropdown: 'value', type: 'trend_field', index: '2',
          });
        }
        break;
      case 'hour':
        for (let i = 0; i < 24; i++) {
          if (i === 0) {
            trendData.push({
              id: `${i}`, label: 'Midnight hour', dropdown: 'value', type: 'trend_field', index: '2',
            });
          } else if (i < 12) {
            trendData.push({
              id: `${i}`, label: `${i} am hour`, dropdown: 'value', type: 'trend_field', index: '2',
            });
          } else if (i === 12) {
            trendData.push({
              id: `${i}`, label: 'Noon hour', dropdown: 'value', type: 'trend_field', index: '2',
            });
          } else {
            trendData.push({
              id: `${i}`, label: `${i - 12} pm hour`, dropdown: 'value', type: 'trend_field', index: '2',
            });
          }
        }
        break;
