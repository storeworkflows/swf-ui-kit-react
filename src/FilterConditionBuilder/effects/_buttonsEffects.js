...
export const BUTTONS_EFFECTS = {
    ...
    deleteCondition: function({ payload }) {
        const { conditionsArray, globalConditionID, currentConditionID, tableFields } = payload;
    ...
    applyQuery: function({ payload }) {
        const { type, conditionsArray, onSendQuery, isSave } = payload;
        let newState;
        switch (type) {
            case "run":
            case "save":
            ...
            newState = QUERY_UTILS.generateQuery({ payload: { conditionsArray: copyConditionArray, onSendQuery, isSave, operation: type } })
            return { ...newState, conditionsArray: copyConditionArray };
        }
    },
    addNewOperator: function({ payload }) {
        const {
            conditionsArray,
            tableFields,
            blockFields,
        ...
        switch(value) {
            case "^":
                ...
                break;
            case '^OR':
                let currentConditionIndexInArr = conditionsArray[globalConditionIndexInArr].relatedConditions.findIndex(cond => cond.id === currentConditionID);
                let parrentConditionOptions = conditionsArray[globalConditionIndexInArr].relatedConditions[currentConditionIndexInArr].conditionOptions;
    ...
    breadcrumbItemClicked: function({ payload }) {
        ...
        switch (operation) {
            case "remove-next":
                ...
                newState = BUTTONS_EFFECTS.deleteCondition({ payload: { tableFields, conditionsArray, currentConditionID: data.conditionId, globalConditionID: data.globalConditionId } });
            ...
            case "remove-subsequent":
                ...
                deletedItems.forEach(item => newState = BUTTONS_EFFECTS.deleteCondition({payload: { currentConditionID: item.conditionId, globalConditionID: item.globalConditionId, tableFields, conditionsArray }}));

                return { ...newState, breadcrumbsItems };
        }
    },
    clickBtn: function({ action, payload }) {
        let newState;
        const {
            conditionsArray,
            blockFields,
        ...
        switch (action) {
            ...
            case "deleteCondition":
                newState = BUTTONS_EFFECTS.deleteCondition({ payload: {...payload, conditionsArray, blockFields, allowFields, tableFields } });
                break;
            case "addNewOperator":
                newState = BUTTONS_EFFECTS.addNewOperator({ payload: {...payload, tableFields, conditionsArray, blockFields, allowFields} });
                break;
            case "applyQuery":
                newState = BUTTONS_EFFECTS.applyQuery({ payload: { ...payload, conditionsArray, isSave, onSendQuery } });
                break;
            case "filterToogle":
            ...
            case "save":
                break;
            case "breadcrumbs":
                newState = BUTTONS_EFFECTS.breadcrumbItemClicked({ payload: { ...payload, breadcrumbsItems, conditionsArray, tableFields } });
                break;
        }
