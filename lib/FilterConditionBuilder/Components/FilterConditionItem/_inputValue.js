...
const _DatePicker = _interopRequireDefault(require('../../../DatePicker/DatePicker'));

const _TextArea = _interopRequireDefault(require('../../../TextArea/TextArea'));
...
const inputValue = function inputValue(_ref) {
  const { inputValuePayload } = _ref;
  const { state } = inputValuePayload;
  const { conditionOptions } = inputValuePayload;
  const { itemClicked } = inputValuePayload;
  const { onDatePickerChange } = inputValuePayload;
  const { inputValueSet } = inputValuePayload;
  const { textAreaValueSet } = inputValuePayload;
  const { lookupFieldValueSet } = inputValuePayload;
  const { dropdownsIsActive } = state;
  const { selectedItem } = state;
  const { generalTable } = state;
  const { refFieldValue } = state;
  const { editor } = conditionOptions.operator;

  const dropdownValueHandle = function dropdownValueHandle(_ref2) {
    const { item } = _ref2;
    const { index } = _ref2;
    const newItem = index ? _objectSpread(_objectSpread({}, item), {}, {
      index,
    }) : _objectSpread({}, item);
    itemClicked(newItem);
  };
