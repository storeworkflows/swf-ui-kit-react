...
const FilterTemplates = /* #__PURE__ */(function (_React$Component) {
  _inherits(FilterTemplates, _React$Component);

  const _super = _createSuper(FilterTemplates);
  ...
  function FilterTemplates(props) {
    ...
    _this.handleSelectItem = function (_ref) {
      ...
      const { clickedItem } = _ref;
      const _this$props = _this.props;
      const { setQuery } = _this$props;
      const { setAdvanced } = _this$props;
      const isAdvanced = ['advanced'].includes(clickedItem === null || clickedItem === void 0 ? void 0 : clickedItem.id);

      _this.setState({
        selectedItem: _objectSpread(_objectSpread({}, _this.state.selectedItem), {}, _defineProperty({}, _this.props.table, clickedItem)),
      });

      const query = isAdvanced ? '' : (_clickedItem$id = clickedItem === null || clickedItem === void 0 ? void 0 : clickedItem.id) !== null && _clickedItem$id !== void 0 ? _clickedItem$id : '';
    ...
    _this.filters = function () {
      const { filterList } = _this.props;
      return [].concat(_toConsumableArray(filterList.map((_ref2) => {
        const { title } = _ref2;
        const { filter } = _ref2;
        return {
          id: filter,
