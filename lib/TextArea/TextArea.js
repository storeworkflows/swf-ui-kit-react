...
const _autosize = _interopRequireDefault(require('autosize/dist/autosize'));
...
const TextArea = function TextArea(props) {
  ...
  const onChangeAction = function onChangeAction(event) {
  ...
  const _onFocus = function onFocus(event) {
    setFocused(true);
  };

  const _onBlur = function onBlur(event) {
    setFocused(false);
    props.onBlur(event);
  };
