...
const TabItem = /* #__PURE__ */React.forwardRef((props, ref) => {
  const infoRef = (0, React.useRef)(null);

  const _useState = (0, React.useState)(false);
  const _useState2 = _slicedToArray(_useState, 2);
  const infoOpened = _useState2[0];
  const setInfoOpened = _useState2[1];
  ...
  const { id } = item;
  const { style } = item;
  const _item$required = item.required;
  const required = _item$required === void 0 ? false : _item$required;
  const _item$invalid = item.invalid;
  const invalid = _item$invalid === void 0 ? false : _item$invalid;
  const { infoMessage } = item;
  const hasIcon = !!icon;
  const hasRef = infoRef && infoRef.current;
  const tabClasses = (0, _classnames.default)({
    'swf-tab': true,
...
const _default = /* #__PURE__ */React.memo(TabItem);
