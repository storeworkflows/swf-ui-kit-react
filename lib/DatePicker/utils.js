...
exports.getErrorOnBoundaryValue = exports.isPointInsideTheElement = exports.addCharToDate = void 0;
...
var getErrorOnBoundaryValue = function getErrorOnBoundaryValue(current, boundary) {
  var isMin = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

  if (current && boundary) {
    var currentDate = new Date(current);
    var boundaryDate = new Date(boundary);
    var invalidCheck = isMin && currentDate < boundaryDate || currentDate > boundaryDate;
    if (invalidCheck) return {
      content: "Date should be ".concat(isMin ? "smaller" : "bigger", " than ").concat(boundaryDate),
