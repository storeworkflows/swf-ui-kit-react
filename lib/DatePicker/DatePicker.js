...
var DatePicker = /*#__PURE__*/function (_React$Component) {
  ...
  function DatePicker(props) {
    ...
    console.log(dateValue && dateValue.format(format));
    _this.state = {
  ...
  _createClass(DatePicker, [{
    ...
    value: function changeValue(e) {
      ...
      if ((0, _moment["default"])(newDateString, format, true).isValid()) {
        var minError = (0, _utils.getErrorOnBoundaryValue)(newDateString, min);
        var maxError = (0, _utils.getErrorOnBoundaryValue)(newDateString, max, false);
    ...
    value: function dateSelected(date) {
      ...
      var dateInFormat = (0, _moment["default"])(date).format(format); // if(!manageValue)
      //{

      var minError = (0, _utils.getErrorOnBoundaryValue)(dateInFormat, min);
      var maxError = (0, _utils.getErrorOnBoundaryValue)(dateInFormat, max, false);

      if (minError || maxError) {
        !manageInvalid && this.invalidInput(true);
        this.setState({
          errorMessages: [minError || maxError]
        });
      } else {
    ...
    value: function checkIsInvalid() {
      ...
      if (!manageInvalid && !isValidValue) errorMessages = [invalidFormatMess];else if (isValidValue) {
        var minError = (0, _utils.getErrorOnBoundaryValue)(stringValue, min);
        var maxError = (0, _utils.getErrorOnBoundaryValue)(stringValue, max, false);
